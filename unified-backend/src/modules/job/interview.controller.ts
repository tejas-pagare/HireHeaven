...
export const evaluateInterview = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (!application_id || !feedback || !tech_rating || !comm_rating || !problem_solving_rating || !culture_rating) {
    throw new ErrorHandler(400, "All rating fields, feedback, and application_id are required");
  }
  ...
  const [evaluation] = await sql`
    INSERT INTO interview_evaluations
      (interview_id, tech_rating, comm_rating, problem_solving_rating, culture_rating, feedback)
    VALUES
      (${interview.interview_id}, ${tech_rating}, ${comm_rating}, ${problem_solving_rating}, ${culture_rating}, ${feedback})
    RETURNING *
  `;
